...
class NotionPipelineError(Exception):
    """Notion連携処理の基底例外。"""

class NotionStructureError(NotionPipelineError):
    """期待する構造が見つからない。"""

class NotionTransientError(NotionPipelineError):
    """一時的エラー（リトライ上限超過）。"""

class NotionPermanentError(NotionPipelineError):
...
def _extract_results(response) -> tuple[list[dict], bool, str | None]:
    if isinstance(response, list):
        return [r for r in response if isinstance(r, dict)], False, None
    if not isinstance(response, dict):
        return [], False, None
    ...
    return [r for r in raw if isinstance(r, dict)], bool(data.get("has_more")), data.get("next_cursor")
...
def _domains_in_order(*email_strs: str) -> list[str]:
    """複数のアドレス文字列からドメインを出現順・重複なしで抽出する。"""
    seen: dict[str, None] = {}
    for s in email_strs:
        for addr in re.findall(r"[\w.+-]+@[\w-]+\.[\w.-]+", s or ""):
            seen.setdefault(_domain_of(addr.lower()), None)
        ...
        if dt is None:
            for fmt in ("%Y/%m/%d %H:%M:%S", "%Y-%m-%d %H:%M:%S",
                ...
                try:
                    ...
                    continue
        if dt is None:
            raise NotionPipelineError(f"日時を解釈できません: {value!r}")
...
async def _get_project(domain: str) -> str | None:
    ...
    results, _, _ = _extract_results(response)
    if not results:
        return None
...
async def save_chat_to_notion(gmail_payload: dict) -> dict:
    ...
    recipient_raw = gp.get("to", "")
    received_at   = gp.get("message_timestamp", "")
    ...
    recipient = _extract_emails(recipient_raw)  # 複数宛先を全て保存
    received_at_jst = _to_jst_str(received_at)

    # 送信トリガーは自分が送信したメールでのみ発火するため、送信者チェックは不要。
    ...
    # 宛先全体（to → cc → bcc の出現順）の全ドメインを候補にして
    # 登録済みドメインを探す。自ドメインは最後に回してクライアント側を優先する
    # （同僚宛＋クライアントCC のメールでもクライアントのプロジェクトが付く）
    own_domain = _domain_of(sender)
    candidates = _domains_in_order(recipient_raw, cc or "", bcc or "")
    ordered = ([d for d in candidates if d != own_domain]
               + [d for d in candidates if d == own_domain])

    project = None
    for domain in ordered:
        project = await _get_project(domain)
        if project is not None:
            break
    if project is None:
        return {"status": "skipped",
                "reason": f"登録済みドメインの宛先が無いため保存しない: {candidates}",
                "message_id": message_id}

    if not message_id:
        raise NotionStructureError("message_id が空のため添付取得・原文特定ができません")

    # name には送信者・受信者両方の表示名を入れる
    names = _extract_names(sender_raw, recipient_raw)
    ...
    att_names, links_parts = await _process_attachments(
        ...
        ts_iso=received_at_jst, sender=sender, names=names, project=project,
    )
    ...
    err = _upsert_failed_detail(inserted)
    if err:
        raise NotionPipelineError(f"Notion書き込み失敗: {err}")

    # 人物辞書の自動更新（送受信者。失敗しても保存結果には影響しない）
    await _upsert_people(_extract_name_email_pairs(sender_raw, recipient_raw))

    return {"status": "success", "id": _upsert_page_info(inserted).get("id"), "message_id": message_id}
